...
//!
//! Aegis renders cursors from **SVG** rather than the legacy Xcursor binary format:
...
//! (`assets/cursors/Aegis`, MIT-licensed art generated by
//! `scripts/prepare-aegis-cursors.py`) so a crisp, standalone cursor always exists
...
//! (viewBox) coordinate space (see `scripts/prepare-aegis-cursors.py`).
...
impl CursorTheme {
    ...
    /// with the bundled Aegis theme so a cursor exists even when nothing is
    /// installed.
