...
fn full_target_damage(size: (u32, u32)) -> Vec<aegis_model::Rect> {
    vec![aegis_model::Rect::new(0, 0, size.0 as i32, size.1 as i32)]
}
...
fn damage_in_target(sampled: &SampledDamage, size: (u32, u32)) -> Vec<aegis_model::Rect> {
    let Some(rects) = sampled.damage.area_rects() else {
        return full_target_damage(size);
    };
    let extent = aegis_model::Rect::new(0, 0, size.0 as i32, size.1 as i32);
    let translated: Vec<aegis_model::Rect> = rects
        .iter()
        .filter_map(|rect| {
...
pub(super) struct WindowStream {
    ...
    pub(super) origin: aegis_model::Point,
    pub(super) logical_size: aegis_model::Size,
...
struct OutputStream {
    ...
    damage_origin: Option<aegis_model::Point>,
...
impl OutputStreams {
    ...
    pub(super) fn cursor_of(&self, stream_id: u64) -> Option<aegis_ipc::StreamCursorMode> {
    ...
    pub(super) fn any_shm_embedded(&self) -> bool {
        self.streams.values().any(|stream| {
            !stream.frozen
                ...
                && stream.cursor == aegis_ipc::StreamCursorMode::Embedded
        })
    }
    ...
    fn sample_damage(&mut self, stream_id: u64, origin: aegis_model::Point) -> SampledDamage {
        let Some(stream) = self.streams.get_mut(&stream_id) else {
...
impl CompositorRuntime {
    ...
    pub(super) fn deliver_stream_frame(&mut self, frame: StreamPixels) {
        ...
        for stream_id in self.streams.due_shm_ids(now) {
            ...
            let cursor_embedded =
                self.streams.cursor_of(stream_id) == Some(aegis_ipc::StreamCursorMode::Embedded);
            ...
            let cropped = |rect: aegis_model::Rect| {
            ...
            let payload = match &target {
                ...
                aegis_ipc::StreamTarget::Window { .. } => continue,
            };
    ...
    pub(super) fn stash_shm_stream_damage(&mut self) {
        ...
        for stream_id in self.streams.due_shm_ids(now) {
            let origin = match self.streams.target_of(stream_id) {
                Some((aegis_ipc::StreamTarget::Output { output: None }, _)) => {
    ...
    pub(super) fn blit_dmabuf_stream_frames(&mut self, acquire_fence: Option<&OwnedFd>) {
        ...
        for stream_id in self.streams.due_dmabuf_ids(now) {
            ...
            let src_rect = match self.streams.target_of(stream_id) {
                ...
                Some((aegis_ipc::StreamTarget::Output { output: None }, size)) => {
                    cursor_blit = cursor_state.as_ref().and_then(|state| {
            ...
            let damage_origin = src_rect
                ...
                .unwrap_or(aegis_model::Point { x: 0, y: 0 });
...
fn output_cursor_blit(
    ...
    target_logical: Option<aegis_model::Rect>,
    scale: f32,
...
fn blit_presented_frame(
    ...
    src_rect: Option<&aegis_model::Rect>,
    ...
    pub(super) fn handle_output_geometry_change(&mut self) {
        ...
        for (stream_id, stream) in &self.streams.streams {
            ...
            let action = match &stream.target {
                ...
                aegis_ipc::StreamTarget::Window { .. } => GeometryAction::Unchanged,
            };
    ...
    pub(super) fn drive_window_streams(&mut self) {
        ...
        for stream_id in ids {
            ...
            let window_id = match stream.target {
                aegis_ipc::StreamTarget::Window { window } => window,
                _ => continue,
            };
            let window = stream
                .window
            ...
            let scale = geometry.scale_milli as f32 / 1000.0;
            let damage_origin = aegis_model::Point {
                x: (geometry.origin.x as f32 * scale).floor() as i32,
                y: (geometry.origin.y as f32 * scale).floor() as i32,
            };
            let cursor_draw = (stream.cursor == aegis_ipc::StreamCursorMode::Embedded)
    ...
    pub(super) fn deliver_window_stream_frame(
        ...
        match pixels {
            Ok(frame) => {
                ...
                let payload = aegis_ipc::StreamFramePayload::Pixels(aegis_ipc::StreamPixelFrame {
                    stream_id,
                    sequence,
                    width: frame.width,
                    height: frame.height,
...
mod tests {
    ...
    fn start(streams: &mut OutputStreams, conn: u64, fps: Option<u32>, size: (u32, u32)) -> u64 {
        streams
            .start(
                ...
                size,
                aegis_ipc::StreamTarget::Output { output: None },
                aegis_ipc::StreamCursorMode::Hidden,
            )
            .stream_id
    }
    ...
    fn cursor_mode_is_stored_on_the_stream_state() {
        ...
        let id = streams
            .start(
                ...
                (100, 100),
                aegis_ipc::StreamTarget::Output { output: None },
                aegis_ipc::StreamCursorMode::Embedded,
            )
            .stream_id;
        assert_eq!(
    ...
    fn resolve_output_rect_maps_and_clamps_by_connector() {
        let outputs = vec![
            ...
            aegis_model::output::OutputInfo {
                ...
                geometry: aegis_model::output::OutputGeometry {
                    ...
                    scale: aegis_model::output::Scale(1.0),
                    transform: aegis_model::Transform::Normal,
                    logical_origin: aegis_model::Point { x: 1920, y: 0 },
                },
                available_modes: Vec::new(),
                color_caps: aegis_model::edid::EdidColorCapabilities::default(),
            },
        ...
        assert_eq!(
            ...
            Some(aegis_model::Rect::new(1920, 0, 2560, 1440))
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(aegis_model::Rect::new(3840, 0, 5120, 2880))
        );
    ...
    fn crop_stream_frame_extracts_rows() {
        ...
        let (w, h, pixels) = crop_stream_frame(4, &bgra, aegis_model::Rect::new(1, 1, 2, 1));
        assert_eq!((w, h), (2, 1));
    ...
    fn attach_stub_window(
        ...
        streams.attach_window(
            ...
            WindowStream {
                shm: None,
                geometry_sig: (0, 0),
                origin: aegis_model::Point { x: 0, y: 0 },
                logical_size: aegis_model::Size { w: 100, h: 50 },
                scale_milli: 1000,
    ...
    fn window_stream_cursor_clips_to_the_window_rect() {
        ...
        let origin = aegis_model::Point { x: 100, y: 50 };
        let size = aegis_model::Size { w: 200, h: 100 };
    ...
    fn damage_translates_and_clips_into_target_space() {
        let sampled = SampledDamage {
            origin: aegis_model::Point { x: 100, y: 50 },
            damage: FrameDamage::Area(vec![
                aegis_model::Rect::new(110, 60, 20, 10),
                // Fully outside the target: clipped away.
                aegis_model::Rect::new(500, 500, 20, 20),
            ]),
        };
        assert_eq!(
            damage_in_target(&sampled, (200, 100)),
            vec![aegis_model::Rect::new(10, 10, 20, 10)]
        );
        // Partially overlapping damage clips to the target extent.
        let sampled = SampledDamage {
            origin: aegis_model::Point { x: 100, y: 50 },
            damage: FrameDamage::Area(vec![aegis_model::Rect::new(90, 40, 20, 20)]),
        };
        assert_eq!(
            damage_in_target(&sampled, (200, 100)),
        ...
        let sampled = SampledDamage {
            origin: aegis_model::Point { x: 0, y: 0 },
            damage: FrameDamage::Area(vec![aegis_model::Rect::new(900, 900, 10, 10)]),
        };
        assert_eq!(
            damage_in_target(&sampled, (200, 100)),
            vec![aegis_model::Rect::new(0, 0, 200, 100)]
        );
    ...
    fn damage_accumulates_until_delivery_and_survives_drops() {
        let mut streams = OutputStreams::new();
        let id = start(&mut streams, 1, Some(30), (100, 100));
        let origin = aegis_model::Point { x: 0, y: 0 };
        ...
        streams.accumulate_damage(&FrameDamage::Area(vec![aegis_model::Rect::new(1, 2, 3, 4)]));
        streams.accumulate_damage(&FrameDamage::Area(vec![aegis_model::Rect::new(5, 6, 7, 8)]));
        let sampled = streams.sample_damage(id, origin);
        assert_eq!(
            sampled.damage,
            FrameDamage::Area(vec![
                aegis_model::Rect::new(1, 2, 3, 4),
                aegis_model::Rect::new(5, 6, 7, 8),
            ])
        );
        ...
        streams.accumulate_damage(&FrameDamage::Area(vec![aegis_model::Rect::new(9, 9, 1, 1)]));
        let sampled = streams.sample_damage(id, origin);
        streams.streams.get_mut(&id).unwrap().note_delivered();
        streams
            .streams
        ...
        assert_eq!(
            ...
            FrameDamage::Area(vec![aegis_model::Rect::new(9, 9, 1, 1)])
        );
    }
    ...
    fn damage_sample_reports_full_after_an_origin_move() {
        let mut streams = OutputStreams::new();
        let id = start(&mut streams, 1, Some(30), (100, 100));
        streams.accumulate_damage(&FrameDamage::Area(vec![aegis_model::Rect::new(1, 1, 5, 5)]));
        let moved = streams.sample_damage(id, aegis_model::Point { x: 50, y: 50 });
        ...
        streams.streams.get_mut(&id).unwrap().note_delivered();
        streams.accumulate_damage(&FrameDamage::Area(vec![aegis_model::Rect::new(2, 2, 5, 5)]));
        let steady = streams.sample_damage(id, aegis_model::Point { x: 50, y: 50 });
        assert_eq!(
            steady.damage,
            FrameDamage::Area(vec![aegis_model::Rect::new(2, 2, 5, 5)])
        );
    }
    ...
    fn any_shm_embedded_tracks_output_shm_streams_only() {
        ...
        streams.streams.get_mut(&window).unwrap().cursor = aegis_ipc::StreamCursorMode::Embedded;
...
mod dmabuf_tests {
    ...
    fn window_stream_render_pipeline_smoke() {
        ...
        let Ok(server) = aegis_compositor::Server::new() else {
            return;
        };
        let mut renderer = aegis_render::Renderer::new();
        let mut cursor_cache = cursor::CursorCache::default();
        let scheme = aegis_model::settings::ColorScheme::Dark;
        let geometry = WindowTreeGeometry {
            window: aegis_model::window::WindowId(1),
            scale_milli: 1000,
            physical_width: 64,
            physical_height: 48,
            origin: aegis_model::Point { x: 0, y: 0 },
            logical_size: aegis_model::Size { w: 64, h: 48 },
        };
