...
fn missing_file_loads_as_none() {
    ...
    assert!(!path.exists());
    assert!(matches!(ass_config::load(&path), Ok(None)));
}
...
fn load_reads_and_parses_a_real_file() {
    ...
    .unwrap();
    let cfg = ass_config::load(&path).unwrap().expect("file should load");
...
fn invalid_file_reports_diagnostics_not_a_crash() {
    ...
    match ass_config::load(&path) {
        Err(ass_config::LoadError::Invalid { diagnostics, .. }) => {
...
#[test]
fn default_path_points_at_ass_config_toml() {
    let Some(p) = ass_config::default_path() else {
