...
use ass_core::realm::{HUMAN_REALM, RealmId, RealmMutation, RealmState};
use ass_core::window::WindowId;
use ass_core::workspace::{Switch, WorkspaceId};
...
impl AssPlatform {
    ...
    fn wait_for_notification(
        ...
        client: &mut Client,
        summary: &str,
    ) -> Result<ass_core::notify::Notification, PlatformError> {
        let deadline = Instant::now() + self.config.io_timeout;
        loop {
