//! ass-control: the command-line driver for the ass IPC.
//!
...
use ass_ipc::{Capabilities, Client, Event, RealmAction, RealmActionResult};
...
fn parse_cli(args: &[String]) -> Result<ParseOutcome, CliError> {
    ...
    full.push("ass-control".to_string());
...
fn dispatch_command(socket: &Path, cli: Cli) -> Result<String, CliError> {
    ...
    match command {
        ...
        Cmd::Minimize { id } => {
            let mut client = Client::connect_with(socket, control_caps()).map_err(connect_err)?;
            client
                .command(ass_ipc::Command::Minimize {
                    id: ass_core::window::WindowId(id),
                })
                .map_err(io_err)?;
        ...
        Cmd::SetGeometry { id, x, y, w, h } => {
            let mut client = Client::connect_with(socket, control_caps()).map_err(connect_err)?;
            let rect = ass_core::Rect::new(x, y, w, h);
            client
                .set_window_geometry(ass_core::window::WindowId(id), rect)
                .map_err(io_err)?;
            Ok(format!("set window {id} geometry to {x},{y} {w}x{h}",))
        }
        Cmd::Switch { direction } => {
            let mut client = Client::connect_with(socket, control_caps()).map_err(connect_err)?;
            let dir: ass_core::workspace::Switch = direction.into();
            client.switch_workspace(dir).map_err(io_err)?;
            Ok(format!("switched {dir:?}"))
        }
        Cmd::SwitchTo { id } => {
            let mut client = Client::connect_with(socket, control_caps()).map_err(connect_err)?;
            let ws = ass_core::workspace::WorkspaceId(id);
        ...
        Cmd::MoveTo { window, workspace } => {
            let mut client = Client::connect_with(socket, control_caps()).map_err(connect_err)?;
            client
                .command(ass_ipc::Command::MoveToWorkspace {
                    window: ass_core::window::WindowId(window),
                    workspace: ass_core::workspace::WorkspaceId(workspace),
                })
                .map_err(io_err)?;
        ...
        Cmd::Overview => {
            let mut client = Client::connect_with(socket, control_caps()).map_err(connect_err)?;
            client
                .command(ass_ipc::Command::ToggleOverview)
                .map_err(io_err)?;
        ...
        Cmd::Completions { shell } => {
            ...
            clap_complete::generate(shell, &mut cmd, "ass-control", &mut std::io::stdout());
            Ok(String::new())
        }
        Cmd::Quit => {
            let mut client = Client::connect_with(socket, control_caps()).map_err(connect_err)?;
            client.command(ass_ipc::Command::Quit).map_err(io_err)?;
...
fn dispatch_realm(socket: &Path, action: RealmCmd, json: bool) -> Result<String, CliError> {
    ...
    let mut client = Client::connect_scoped(socket, caps, ass_ipc::LOCAL_REALM_ADMIN_SCOPE)
        .map_err(connect_err)?;
    match action {
        RealmCmd::List => {
        ...
        RealmCmd::Transfer {
            ...
            let realm = validate_realm_id(realm)?;
            let window = ass_core::window::WindowId(window);
            let snapshot = client.realms().map_err(io_err)?;
            let result = client
                .realm_action(RealmAction::Transact {
                    expected_revision: Some(snapshot.revision),
