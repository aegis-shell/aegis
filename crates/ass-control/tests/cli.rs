...
//! test starts a server with a fixed handler, invokes `ass_control::run` with a
...
fn scratch() -> PathBuf {
    ...
    p.push(format!("ass-control-{}-{n}.sock", std::process::id()));
    p
}
...
fn windows_command_lists_the_fixed_window() {
    ...
    let out = ass_control::run(&path, &["windows".into()]).unwrap();
...
fn json_flag_emits_parseable_json_for_windows() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = ass_control::run(&path, &["windows".into(), "--json".into()]).unwrap();
...
fn workspaces_command_shows_output_and_workspace() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = ass_control::run(&path, &["workspaces".into()]).unwrap();
...
fn outputs_command_lists_advertised_modes_and_marks_current() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = ass_control::run(&path, &["outputs".into()]).unwrap();
...
fn notifications_command_lists_active_notifications() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = ass_control::run(&path, &["notifications".into()]).unwrap();
...
fn journal_command_lists_entries() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = ass_control::run(&path, &["journal".into(), "2".into()]).unwrap();
...
fn realms_command_uses_admin_scope_and_lists_agent_realm() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = ass_control::run(&path, &["realm".into(), "list".into()]).unwrap();
...
fn realm_create_and_transfer_use_optimistic_actions() {
    ...
    let created =
        ass_control::run(&path, &["realm".into(), "create".into(), "Builder".into()]).unwrap();
    assert!(created.contains("created Realm 2"), "{created}");
    let transferred = ass_control::run(
        &path,
...
fn focus_command_sends_focus() {
    ...
    let out = ass_control::run(&path, &["focus".into(), "1".into()]).unwrap();
...
fn minimize_command_sends_minimize() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    let out = ass_control::run(&path, &["minimize".into(), "1".into()]).unwrap();
...
fn switch_command_sends_workspace_switch() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    ass_control::run(&path, &["switch".into(), "next".into()]).unwrap();
    assert!(
        h.commands
            .lock()
            .unwrap()
...
fn switch_to_command_sends_direct_workspace_switch() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    let out = ass_control::run(&path, &["switch-to".into(), "9".into()]).unwrap();
...
fn binary_prints_query_output() {
    ...
    let output = std::process::Command::new(env!("CARGO_BIN_EXE_ass-control"))
...
fn notify_command_sends_notify_with_body() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    ass_control::run(&path, &["notify".into(), "Hello".into(), "world".into()]).unwrap();
    assert!(
...
fn dismiss_command_sends_dismiss_notification() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    let out = ass_control::run(&path, &["dismiss".into(), "7".into()]).unwrap();
...
fn tiling_command_sends_toggle() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    ass_control::run(&path, &["tiling".into()]).unwrap();
    assert!(
        h.commands
            .lock()
            .unwrap()
...
fn move_to_command_sends_move_to_workspace() {
    ...
    let h = Arc::new(CtlHandler::new());
    let _s = Server::start(&path, Arc::clone(&h)).unwrap();
    let out = ass_control::run(&path, &["move-to".into(), "42".into(), "3".into()]).unwrap();
...
fn unknown_command_errors_with_usage() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let err = ass_control::run(&path, &["bogus".into()]).unwrap_err();
...
fn help_command_returns_usage() {
    let path = scratch();
    let _s = Server::start(&path, Arc::new(CtlHandler::new())).unwrap();
    let out = ass_control::run(&path, &["help".into()]).unwrap();
