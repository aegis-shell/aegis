...
use aegis_lock::{AuthResult, LockAction, LockScreenStyle, LockState, PresentationMode};
...
const APP_ID: &str = "dev.aegis.LockPreview";
const TITLE: &str = "Aegis Lock Preview";
...
struct Options {
    ...
    password: Option<aegis_lock::Secret>,
    size: (u32, u32),
...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    let options = match parse_args(std::env::args().skip(1))? {
        ...
        ParsedCommand::Version => {
            println!("aegis-lock-preview {}", env!("CARGO_PKG_VERSION"));
            return Ok(());
        }
    };
...
fn simulated_auth_result(
    result: SimulatedResult,
    submitted: &aegis_lock::Secret,
    expected: Option<&aegis_lock::Secret>,
) -> AuthResult {
...
fn parse_preview_password(value: &str) -> Result<aegis_lock::Secret, String> {
    if value.is_empty() {
        return Err("preview password must not be empty".into());
    }
    let mut password = aegis_lock::Secret::default();
    if !password.push_str(value) {
        return Err(format!(
            "preview password must not exceed {} UTF-8 bytes",
            aegis_lock::Secret::MAX_BYTES
        ));
    }
    Ok(password)
}
...
fn print_help() {
    print!(
        "{}",
        concat!(
            "Usage: aegis-lock-preview [OPTIONS]\n\n",
