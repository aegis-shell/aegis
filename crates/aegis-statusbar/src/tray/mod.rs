...
//! watcher already owns the name, [`spawn`] returns `None` and the bar simply
//! shows no SNI icons.
//!
...
//! the render thread paints a popover from it, and clicks travel back through
...
/// targeted submenu id no longer exists (the bar should pop back to the
/// nearest valid ancestor).
pub(crate) fn visible_children<'a>(root: &'a MenuNode, path: &[i32]) -> Option<&'a [MenuNode]> {
    let mut node = root;
