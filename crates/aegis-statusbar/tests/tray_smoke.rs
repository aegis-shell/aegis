...
use aegis_statusbar::tray::{self, TrayCommand, TrayIcon};
...
fn watcher_registers_item_and_delivers_activate() {
    let Some((snapshot, commands)) = tray::spawn() else {
    ...
    // A left click from the bar reaches the item as Activate(x, y).
    commands
