//! Shared local-account identity and portrait source contract for Aegis.
//!
//! Every consumer uses the same [`Identity`], [`PortraitConfig`], source
//! precedence, still-image normalization, and transactional watcher. VRM
//! parsing and animation remain delegated to `aegis-avatar`; presentation
//! hosts own fallback visuals and surrounding chrome.
...
use aegis_avatar::{AnimationSupport, MotionInfo, VrmCamera};
use flux::{Device, Image};

/// Errors raised while resolving or preparing shared portrait content.
#[derive(Debug, thiserror::Error)]
pub enum Error {
    #[error("identity portrait: decode {0:?}")]
    Decode(PathBuf, #[source] image::ImageError),
    #[error("identity portrait: {0}")]
    Avatar(#[from] aegis_avatar::Error),
    #[error("identity portrait: texture upload failed: {0}")]
    Flux(#[from] flux::Error),
    #[error("identity portrait: configured sources exist but none could be decoded")]
    NoUsableSource,
}
...
enum Content {
    Still(Image),
    Vrm(Box<aegis_avatar::Avatar>),
}

/// Prepared portrait content selected through the shared identity contract.
pub struct Portrait {
    content: Content,
}
        ...
        for candidate in config.candidates() {
            match candidate {
                ...
                PortraitCandidate::Vrm {
                    ...
                    match aegis_avatar::Avatar::load(device, model, legacy_motion, camera) {
                        Ok(avatar) => {
                            return Ok(Some(Self {
                                content: Content::Vrm(Box::new(avatar)),
                            }));
                        }
                        Err(aegis_avatar::Error::Io(path, _)) if path == *model => continue,
                        Err(aegis_avatar::Error::Gltf(path, _)) if path == *model => continue,
