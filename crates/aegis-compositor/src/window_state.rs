...
//! this store. Keeping the JSON adapter here leaves `aegis-model` limited to
...
pub(crate) struct SavedWindowState {
    ...
    pub(crate) position: Option<aegis_model::Point>,

    /// Saved size in compositor logical coordinates.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub(crate) size: Option<aegis_model::Size>,
    ...
    /// Saved layout role.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub(crate) layout_role: Option<aegis_model::layout::LayoutRole>,
...
mod tests {
    ...
    fn update_and_prune_preserve_bounded_state() {
        ...
        store.update(
            ...
            SavedWindowState {
                position: Some(aegis_model::Point { x: 100, y: 200 }),
                size: Some(aegis_model::Size { w: 1024, h: 768 }),
                workspace: Some(2),
                layout_role: Some(aegis_model::layout::LayoutRole::Floating),
        ...
        assert_eq!(
            retrieved.position,
            Some(aegis_model::Point { x: 100, y: 200 })
        );
        assert_eq!(retrieved.size, Some(aegis_model::Size { w: 1024, h: 768 }));
