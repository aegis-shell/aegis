...
// onto the same clipboard: aegis models exactly one selection per seat. The
...
static DATA_CONTROL_DEVICE_IMPL: ffi::ext_data_control_device_v1_interface_impl =
    ffi::ext_data_control_device_v1_interface_impl {
        ...
        destroy: res_destroy,
        // aegis models exactly one selection per seat (no independent
...
/// `set_primary_selection`: aegis models exactly one selection per seat (no
