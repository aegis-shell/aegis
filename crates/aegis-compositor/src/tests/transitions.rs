...
fn closing_frame_interpolates_then_settles() {
    let rect = aegis_model::Rect::new(100, 100, 800, 600);
    ...
    let mid = frame
        .rect_at(1000 + aegis_model::transition::CLOSE_DURATION_MS / 2)
        .expect("mid flight");
    ...
    let mid_opacity = frame
        .opacity_at(1000 + aegis_model::transition::CLOSE_DURATION_MS / 2)
        .expect("mid flight opacity");
    assert!(mid_opacity > 0.0 && mid_opacity < 1.0);

    let deadline = 1000 + aegis_model::transition::CLOSE_DURATION_MS;
...
fn inset_rect_targets_sixteenth_and_keeps_floor() {
    let rect = aegis_model::Rect::new(0, 0, 160, 96);
    ...
    let tiny = inset_rect(aegis_model::Rect::new(0, 0, 3, 3), 50, 50);
    assert_eq!(tiny.size, aegis_model::Size { w: 2, h: 2 });
}
