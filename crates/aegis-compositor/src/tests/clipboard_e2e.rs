...
fn wl_clipboard_roundtrips_without_creating_a_window() {
    ...
    let copy = std::process::Command::new("wl-copy")
        ...
        .arg("aegis-ext-data-control-probe")
...
fn clipboard_probe_binary() -> Option<std::path::PathBuf> {
    ...
    let out = std::env::temp_dir().join(format!("aegis-clip-probe-{}", std::process::id()));
...
/// aegis has no independent primary clipboard, so `set_primary_selection`
