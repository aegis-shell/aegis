...
use ass_control_center::builtin_settings_modules;
use ass_control_center::module::{ModuleAvailability, ModuleEvents, ModuleId, ModuleRegistry};
use ass_core::settings::{SettingsAction, SettingsSnapshot};
use ass_design::{Design, themes};
use ass_ipc::{Capabilities, Client};
use ass_shell::{Localizer, Message};
...
fn module_label(i18n: &Localizer, module: ass_control_center::module::ModuleMetadata) -> String {
...
fn socket_path() -> Result<PathBuf, String> {
    std::env::var_os("XDG_RUNTIME_DIR")
        .map(PathBuf::from)
        .map(|runtime| runtime.join("ass.sock"))
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let config = Config::new(app.i18n.text(Message::ControlCenter))?
        .app_id(ass_core::app::CONTROL_CENTER_APP_ID)?
        .size(920, 680)
        .force_dark();
