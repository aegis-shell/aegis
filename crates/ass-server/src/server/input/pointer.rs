...
impl Server {
    ...
    pub(crate) fn pointer_button(&mut self, button: u32, state: ass_core::input::ButtonState) {
    ...
    pub(crate) fn post_motion_to_focus(&self) {
        if self.state.pointer_focus.is_null() {
            return;
        }
        let focus = self.state.pointer_focus;
    ...
    pub(crate) fn pointer_axis(&mut self, frame: ass_core::input::PointerAxisFrame) {
