...
fn scratch() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("aegis-fuji-bridge-{}-{n}", std::process::id()))
}
...
fn fuji_stdio_discovers_manages_captures_and_revokes_realm() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_aegis-fuji-mcp"))
        .arg("serve")
        .env("XDG_RUNTIME_DIR", &runtime_dir)
        .env("AEGIS_FUJI_SCOPE", "fuji-test")
        .env("AEGIS_FUJI_REALM_LABEL", "Fuji integration test")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
