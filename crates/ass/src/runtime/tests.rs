...
fn only_user_initiated_screenshots_update_the_human_clipboard() {
    assert!(screenshot_updates_human_clipboard(ass_ipc::Origin::Chrome));
    assert!(screenshot_updates_human_clipboard(
        ass_ipc::Origin::Keybinding
    ));
    assert!(!screenshot_updates_human_clipboard(ass_ipc::Origin::Ipc {
        conn_id: 7,
    }));
    assert!(!screenshot_updates_human_clipboard(
        ass_ipc::Origin::Internal
    ));
}
