...
pub(super) fn physical_window_target(cmd: &ass_ipc::Command) -> Option<ass_core::window::WindowId> {
    use ass_ipc::Command;
    match cmd {
        Command::Focus { id }
        | Command::Minimize { id }
...
pub(super) fn realm_intent_to_action(intent: ass_shell::RealmIntent) -> ass_ipc::RealmAction {
    match intent {
