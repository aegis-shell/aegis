...
impl PersistentConnection {
    ...
    /// first connect should defer calling this, as `aegis-mcp`'s lazy
    /// platform does.
...
pub enum PersistentError<E> {
    ...
    #[error("Aegis IPC request failed: {0}")]
    Call(E),
}
...
mod tests {
    ...
    use aegis_ipc::{ActorCapability, ConnectionCapabilities, Handler, Server};
    ...
    impl Handler for TestHandler {
        ...
        fn agent_lookup(&self, credential: &str) -> Option<aegis_ipc::AgentIdentity> {
            (credential == "cred_test").then(|| aegis_ipc::AgentIdentity {
    ...
    fn scratch() -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("aegis-ipc-client-{}-{n}.sock", std::process::id()))
    }
