...
`aegis-prism` is the compact, Spotlight-style application search component
for the Aegis compositor. The default `Super+Space` shortcut opens it without
...
- Report its glass backdrop region, modal input ownership, and animation
  state through the `aegis-shell` `Chrome` contract.

## Boundaries

Prism owns presentation and interaction state only. The `aegis` binary owns
