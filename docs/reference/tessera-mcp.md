# tessera-mcp Bridge Reference

`tessera-mcp` is the scoped Tessera platform bridge: the standard MCP access
...
tessera-mcp [serve]
tessera-mcp check
tessera-mcp smoke [--observe-seconds <1..=30>] [--input-window <id>]
tessera-mcp print-config
```
...
`[mcp.tessera]` entry using the current executable path.
...
| `tessera interaction-domain list` | Shows each Interaction Domain's authority revision, state, seat, and controlled-window count. |
...
|----------|---------|-------------|
| `TESSERA_MCP_SOCKET` | `$XDG_RUNTIME_DIR/tessera.sock` | Tessera IPC socket. |
| `TESSERA_MCP_LABEL` | the Interaction Domain label | Cosmetic display label presented at pairing; set it per agent product so principals stay distinguishable. 1–128 bytes. |
| `TESSERA_MCP_INSTANCE_ID` | required | Stable connector-installation namespace for the credential, lock, and recovery state. `print-config` emits a random value; keep it stable and unique per MCP entry. 1–128 bytes. |
| `TESSERA_MCP_INTERACTION_DOMAIN_LABEL` | `Tessera Agent` | Cosmetic label used for this bridge's Agent Interaction Domain; 1–128 bytes. Recovery also requires the authenticated subject binding. |
| `TESSERA_MCP_DATA_DIR` | `$XDG_DATA_HOME/tessera-mcp` | Durable directory for the pairing identity. Without a resolvable data directory the identity is session-only and every start re-pairs. |
| `TESSERA_MCP_IPC_TIMEOUT_SECS` | `5` | Per-connection handshake and I/O timeout for query calls; valid range `1..=60`. Mutation calls that may block on a grant prompt use a 360-second bound. |
| `TESSERA_MCP_REVOKE_ON_EXIT` | `false` | Revoke the managed Interaction Domain after graceful MCP EOF. Prefer explicit `interaction_domain_reset`; enabling this may wait for runtime consent during shutdown. |
...
`$XDG_RUNTIME_DIR/tessera-mcp/` directory; the pairing identity lives in the
...
is nothing to configure on the Tessera side. On first contact the bridge
...
> **"Tessera Agent" wants to borrow desktop capabilities**
...
- **Allow session** applies until the compositor restarts or the decision is
  revoked by the user.
- **Always allow** persists in `$XDG_DATA_HOME/tessera/grants.json` until
  revoked.

Manage everything with `tessera permissions` (`list`, `revoke`, `forget`,
...
With the server key `tessera`, an MCP client that namespaces tools exposes
the public tool names as `mcp__tessera__<tool>`.
