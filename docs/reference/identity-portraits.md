...
The shared identity portrait contract keeps account metadata, still images,
VRM sources, and live reload consistent across presentation surfaces.
...
The shared identity layer decodes PNG, JPEG, WebP, GIF, BMP, ICO, TIFF, TGA,
QOI, and PNM. It cover-fits the first decoded frame to a square, applies a
premultiplied circular mask, and uploads one `RGBA8_UNORM` texture.

Still support is not part of the VRM renderer. Removing still decoding from
`aegis-avatar` therefore does not remove still portraits from Aegis.
...
composition values. A caller can apply a new profile with
`Avatar::set_camera`; unchanged values do not trigger GPU work.
