# Tessera and Optics Cross-Repository Development

Use one long-lived linked Git worktree for Tessera and Optics development. The
primary Tessera worktree remains on `main` with the canonical remote dependency
...
| Concern | Primary worktree | Tessera development worktree |
|---------|------------------|-----------------------------|
...
Create the linked worktree once from the primary Tessera worktree:
...
The primary `tessera/` worktree keeps `main` checked out. The `tessera-dev/`
...
Do not create `.cargo/config.toml` in the primary Tessera worktree. Confirm that
...
Compile Optics before building an Tessera consumer:

```bash
meson compile -C ../optics/build
cargo check --locked -p tessera
...
Do not use `--no-verify`. Local patch state is not part of an Tessera commit.
...
Land and tag Optics before making the Tessera branch canonical. Use one
...
Disable local mode and restore the canonical Tessera lockfile:

```bash
mv .cargo/config.toml /tmp/tessera-optics-local.toml
git restore Cargo.lock
```
...
cd ../tessera
git worktree remove ../tessera-dev
```

## Test an Unreleased Optics Commit in CI

When Tessera CI must run before an Optics tag exists, temporarily point every
Optics Git dependency at the same fixed `rev`. Do not use a moving branch.
Replace the fixed revision with the final release tag before merging Tessera.
...
(matching the Tessera table above), the aphrodite development worktree
