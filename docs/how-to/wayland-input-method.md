...
ass supports native Wayland input methods on the physical seat. Applications
...
Run the registry inspection inside the ass session:
...
virtual-keyboard manager is absent after updating ass, restart the compositor.
...
`zwp_text_input_manager_v3`. X11 applications are outside ass's supported
