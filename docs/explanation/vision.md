...
aegis has two sequential goals. The first is a good desktop for human users.
...
shortcuts, unlogged mutations — that an agent cannot reach. aegis keeps that
...
aegis reaches it with a fraction of the code and a single coherent model.
...
lives once, in `aegis-model`. The renderer, the chrome, the IPC, and the later
...
schema, full live reload. aegis does not embed a scripting language. Power and
...
**Rendering and UI are delegated.** flux renders; lens draws the chrome. aegis
...
**Wayland-only.** aegis will never ship an X11 session, and XWayland is
...
records what aegis takes and what it leaves.
...
- The Wayland server, the seat, the output and input pipelines, and the
  window manager, owned directly by aegis
...
To stay small, aegis explicitly does not aim to provide the following:

- **X11 applications.** aegis is Wayland-only; XWayland
...
- **Mobile, tablet, and TV shells as built-in variants.** The `Chrome` trait
  makes such shells possible as separate compositions; aegis ships only the
  desktop composition.
...
- [Comparative Survey](comparative-survey.md) — the systems aegis borrows from
  and rejects.
